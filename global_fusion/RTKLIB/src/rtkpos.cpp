...
{
    // LOG(INFO)<<"sdobs (single difference with f)-> "<< f;
                     ...
                     const prcopt_t *opt)
{
    ...
    // LOG(INFO)<<"c-> "<<c;
    // LOG(INFO)<<"d-> "<<d;
    // LOG(INFO)<<"var-> "<<2.0*(opt->ionoopt==IONOOPT_IFLC?3.0:1.0)*(a*a+b*b/sinel/sinel+c*c)+d*d;
    // LOG(INFO)<<"\n "<<d;
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    // LOG(INFO)<<"temporal update of phase biases ";
    ...
    for (f=0;f<nf;f++) { // tranverse frequency (L1, L2, L5)
        ...
        for (i=j=0,offset=0.0;i<ns;i++) {
            ...
            if (rtk->opt.ionoopt!=IONOOPT_IFLC) {
                ...
                LOG(INFO) << "estimate approximate phase-bias by phase - code";
            }
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (j=0;j<ns;j++) {
            ...
            if (opt->ionoopt==IONOOPT_EST) {
                LOG(INFO) <<"double-differenced ionospheric delay term";
            ...
            if (opt->tropopt==TROPOPT_EST||opt->tropopt==TROPOPT_ESTG) {
                LOG(INFO) <<"double-differenced tropospheric delay term";
            ...
            if (f<nf) {
                // LOG(INFO) <<"double-differenced phase-bias term";
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    if (!(info=lambda(nb,2,y+na,Qb,b,s))) {
        ...
        if (s[0]<=0.0||s[1]/s[0]>=opt->thresar[0]) {
            LOG(INFO) <<"opt->thresar[0]-> " <<opt->thresar[0];
                  ...
                  const nav_t *nav)
{
    ...
    double *rs; // satellite position and velocity
    double *dts; // satellite clock bias
    double *var;
    double *y,*e,*azel,*v,*H,*R,*xp,*Pp,*xa,*bias,dt;
    ...
    dt=timediff(time,obs[nu].time); // time difference between user and ref
    
    /*rs [(0:2)+i*6]= obs[i] sat position {x,y,z} (m)
      rs [(3:5)+i*6]= obs[i] sat velocity {vx,vy,vz} (m/s)*/
    rs=mat(6,n); 
    dts=mat(2,n); //dts[(0:1)+i*2]= obs[i] sat clock {bias,drift} (s|s/s)
    var=mat(1,n); 
    y=mat(nf*2,n); // measurement nf: number of frequency
    e=mat(3,n);
    azel=zeros(2,n); // azimuth/elevation angles 
    
    for (i=0;i<MAXSAT;i++) {
        rtk->ssat[i].sys=satsys(i+1,NULL);
        for (j=0;j<NFREQ;j++) rtk->ssat[i].vsat[j]=rtk->ssat[i].snr[j]=0;
    }
    /* satellite positions/clocks both the satellite from user and ref sta*/
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /* undifferenced residuals for base station */
    // Weisong: check the feasibility of satellites
    if (!zdres(1,obs+nu,nr,rs+nu*6,dts+nu*2,svh+nu,nav,rtk->rb,opt,1,
               y+nu*nf*2,e+nu*3,azel+nu*2)) {
        errmsg(rtk,"initial base station position error\n");
        LOG(INFO) <<"initial base station position error";
    ...
    if (opt->intpref) {
        ...
        LOG(INFO) <<"time-interpolation of residuals (for post-processing)";
    }
    ...
    /* Weisong: publish float solution */
    #if 1  // pub_rtkpos_odometry_integer
    ...
    // for(int i = 0; i < 6; i++)
    // {
    //     odometry.pose.covariance[i] = rtk->sol.qr[i];
    // }
    odometry.twist.twist.linear.x = rtk->sol.rr[3];
    odometry.twist.twist.linear.y = rtk->sol.rr[4];
    odometry.twist.twist.linear.z = rtk->sol.rr[5];
    pub_rtkpos_odometry_float.publish(odometry);
    #endif
    ...
    /* Weisong: publish integer solution */
    #if 1  // 
    // Eigen::Matrix<double, 3,1> ENU_ref;
    // ENU_ref<< 114.190297420,22.301487386,0;
    // ENU_ref<< ref_lon, ref_lat, ref_alt;
    ...
    odometry.pose.pose.position.y = ENU(1);
    odometry.pose.pose.position.z = 1;
    pub_rtkpos_odometry_integer.publish(odometry);
    #endif
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
     ...
     * the satellite number from rover and base should be different, 
     * due to the different sky view
    */
    for (nu=0;nu   <n&&obs[nu   ].rcv==1;nu++)
    {
        ...
        // LOG(INFO) <<"obs[nu].P[0]->  "<<obs[nu].P[0];
        // LOG(INFO) <<"obs[nu].P[1]->  "<<obs[nu].P[1];
        // LOG(INFO) <<"obs[nu].P[2]->  "<<obs[nu].P[2];
        // LOG(INFO)<< "rtk-opt.nf->    "   <<rtk->opt.nf <<"   " <<NFREQ+NEXOBS;
    }
    for (nr=0;nu+nr<n&&obs[nu+nr].rcv==2;nr++)
    {

        // LOG(INFO) <<"sat id->  "<<float(obs[nu+nr].sat);
        // LOG(INFO) <<"obs[nu+nr].L[0]->  "<<obs[nu+nr].L[0];
        // LOG(INFO) <<"obs[nu+nr].L[1]->  "<<obs[nu+nr].L[1];
        // LOG(INFO) <<"obs[nu+nr].L[2]->  "<<obs[nu+nr].L[2];
        // LOG(INFO) <<"obs[nu+nr].P[0]->  "<<obs[nu+nr].P[0];
        // LOG(INFO) <<"obs[nu+nr].P[1]->  "<<obs[nu+nr].P[1];
        // LOG(INFO) <<"obs[nu+nr].P[2]->  "<<obs[nu+nr].P[2];
        // LOG(INFO) <<"\n";
        
    }

    // LOG(INFO) <<"nu-> "<<nu; // user end received satellites
    // LOG(INFO) <<"nr-> "<<nr; // reference end received satellites
    
    time=rtk->sol.time; /* previous epoch */
   

    // LOG(INFO) <<"gps time seconds-> "<<obs[0].time.sec;
    int current_week;
    double current_tow;
    current_tow = time2gpst(obs[0].time, &current_week);
    // if(current_tow<46699) sleepms(1);
    if(current_tow<start_gps_sec) sleepms(1);
    else 
        sleepms(delayms);
    ...
    if (!pntpos(obs,nu,nav,&rtk->opt,&rtk->sol,NULL,rtk->ssat,msg)) {
        ...
        // LOG(INFO) <<"warming in pntpos calculation!!! "<<msg;
        if (!rtk->opt.dynamics) {
            // LOG(INFO) <<"Warming! Chi-square Failure Finish with SPP (PVT)!!! "<<current_tow;
            // sleepms(delayms);
            // outsolstat(rtk);
            // return 0;
        }
    ...
    // LOG(INFO) <<"rtk->sol.rr[0] using PNT-> "<<rtk->sol.rr[0];
    // LOG(INFO) <<"rtk->sol.rr[1] using PNT-> "<<rtk->sol.rr[1];
    // LOG(INFO) <<"rtk->sol.rr[2] using PNT-> "<<rtk->sol.rr[2];
    // LOG(INFO) <<"rtk->sol.rr[3] using PNT-> "<<rtk->sol.rr[3];
    // LOG(INFO) <<"rtk->sol.rr[4] using PNT-> "<<rtk->sol.rr[4];
    // LOG(INFO) <<"rtk->sol.rr[5] using PNT-> "<<rtk->sol.rr[5];


    if (time.time!=0) rtk->tt=timediff(rtk->sol.time,time); // the time in rtk->sol.time is updated by the function pntpos()
    ...
    if (opt->mode==PMODE_MOVEB) { /*  moving baseline */
        ...
        /* the time difference between the observation from user and reference*/
        rtk->sol.age=(float)timediff(obs[0].time,obs[nu].time);
        LOG(INFO) <<"age-> " << rtk->sol.age;
    ...
    // LOG(INFO)<<"Warming! Chi-square Success Finish with RTK solution!!!"<<current_tow;
    // ROS_INFO("\033[1;32m----> finish epoch.\033[0m");
    // if(current_tow<46699) sleepms(2);
    if(current_tow<start_gps_sec) sleepms(2);
    else 
        sleepms(delayms);
